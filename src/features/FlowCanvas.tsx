...
import {
  ...
  TEXT_NODE_TYPE,
  type TextNodeData,
} from "../components/nodes/TextNode";
...
export default function FlowCanvas() {
  ...
  const onConnect: OnConnect = useCallback(
    (params: Connection) => {
      ...
      if (alreadyHasOutgoing) {
        window.alert(
          "Only one connection allowed from the source handle of a node."
        );
        return;
      }
  ...
  const updateSelectedNodeText = useCallback(
    (text: string) => {
      if (!selectedId) return;
      setNodes((nds) =>
        nds.map((n) =>
          n.id === selectedId
            ? ({ ...n, data: { ...n.data, text } } as RFNode)
            : n
        )
      );
    },
  ...
  const computeIncomingCounts = useCallback(() => {
    ...
    edges.forEach((e) => {
      if (e.target) map.set(e.target, (map.get(e.target) || 0) + 1);
    });
    return map;
  }, [nodes, edges]);
    ...
    setNodes((nds) =>
      nds.map(
        (n) =>
          ({
            ...n,
            data: { ...n.data, isRoot: roots.includes(n.id) },
          } as RFNode)
      )
    );
  ...
  const validateAndSave = useCallback(() => {
    try {
      if (nodes.length === 0) {
        toast.error("Cannot save an empty flow. Please add at least one node.");
        return;
      }
      const incoming = computeIncomingCounts();
      const roots = Array.from(incoming.entries())
        .filter(([_, c]) => c === 0) // no incoming edges
        .map(([id]) => id);

      // 🚨 Validation
      if (nodes.length > 1 && roots.length > 1) {
        toast.error(
          `Flow invalid: ${roots.length} nodes have no incoming edges (multiple entry points).`
        );
        return;
      }
      ...
      console.error(err);
      toast.error("Failed to save the flow. Please try again.");
    }
      ...
      <div className="grid grid-cols-[300px_1fr] min-h-0">
        <aside className="sidebar">
          {selectedId ? (
            <SettingsPanel
              key={selectedId}
              value={nodes.find((n) => n.id === selectedId)?.data.text || ""}
              onChange={updateSelectedNodeText}
